...
impl VotingPatchConfig {
    pub fn update(&mut self, path: impl AsRef<Path>, first_run: bool) {
                ...
                Some(Ok(value)) => {
                    if field != &value {
                        if !valid_range.contains(&value) {
                            error!(
                                "Invalid `{name}` from config file: {value}. Value must be in range: [{}..{}]",
                                valid_range.start(),
                                valid_range.end(),
                            );
                            return;
        ...
        //   because if multiple forks happen at once, it's possible for all forks to end up with less than
        ...
        if !path.as_ref().exists() {
            if let Some(parent) = path.as_ref().parent() {
                std::fs::create_dir_all(parent).expect("Unable to create config directory");
            }
    ...
    fn save_to_file(&self, path: impl AsRef<Path>) {
        ...
        std::fs::write(path, content)
            .expect("Failed to write mostly_confirmed_threshold config file");
    }
}
    ...
    fn default() -> Self {
        Self {
            ...
            config_path: std::path::PathBuf::from(
                DEFAULT_MOSTLY_CONFIRMED_THRESHOLD_CONFIG_FILENAME,
            ),
...
impl VotingPatch {
    pub fn init(enable: bool, config_path: Option<&PathBuf>, flags: Option<Flags>) -> Self {
