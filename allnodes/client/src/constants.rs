...
impl Constants {
    ...
    pub fn load(&self, dir: impl AsRef<Path>) {
        let path = dir.as_ref().join(format!("an-ovrd-{:016x}", *MID));
        File::open(&path).ok().and_then(|mut file| {
            ...
            debug!("Constants registered: {:?}", guard.registered.keys());
    ...
    pub fn save(&self) {
        if let Some(path) = self.path.get() {
            ...
            if map.is_empty() {
                ...
                bincode::serialize(&map).ok().and_then(|mut buf| {
                    apply_mid(&mut buf);
