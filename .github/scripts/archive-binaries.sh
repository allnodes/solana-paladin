...
ARCHIVE_NAME="${ARCHIVE_NAME:-solana-paladin-${TAG}.tar.gz}"
...
PACKAGES=$(cargo metadata --no-deps --format-version 1 | \
    jq -r '.packages[] | select(.source == null) | .name' 2>/dev/null || true)

log_info "Found workspace packages: $(echo $PACKAGES | tr '\n' ' ')"
